...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
            ...
            if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
                ...
                UsernamePasswordAuthenticationToken authentication =
                        new UsernamePasswordAuthenticationToken(
                                userDetails,
                                null, // Пароль здесь не нужен, так как пользователь уже аутентифицирован по токену
        ...
        filterChain.doFilter(request, response); // Передаем запрос следующему фильтру в цепочке
    }
    ...
    private String parseJwt(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(headerAuth) && headerAuth.startsWith("Bearer ")) {
            return headerAuth.substring(7); // "Bearer " имеет длину 7
        }
        return null;
    }
}